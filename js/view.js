const view = {
  ...
  addControlToken: function (player, state, times) {
    const board = document.getElementById("board");
    for (let i = 0; i < times; i++) {
      const token = document.createElement("div");
      token.classList.add("controlToken");
      token.classList.add("p-" + player);
      token.style.left = `${state.x - 15 + i * 5}px`;
      token.style.top = `${state.y - 15 + i * 5}px`;
      board.appendChild(token);
    }
  },
  ...
  addBonusToken: function (type, state, index) {
    const board = document.getElementById("board");
    const token = document.createElement("div");
    token.classList.add("bonusToken");
    token.classList.add("bonus-" + type);
    token.id = "bonus-" + index;
    token.style.left = `${state.x - 13}px`;
    token.style.top = `${state.y - 13}px`;
    board.appendChild(token);
  },
  ...
  addInfluenceToken: function (player, border, influence, direction) {
    const board = document.getElementById("board");
    const token = document.createElement("div");
    token.classList.add("influenceToken");
    token.classList.add("p-" + player);
    token.id = "it-" + border.id;
    token.style.left = `${border.location.x - 16}px`;
    token.style.top = `${border.location.y - 16}px`;
    token.style.transform = `rotate(${border.rotation}deg)`;
    const p1 = document.createElement("p");
    const p2 = document.createElement("p");
    if (direction) {
      p1.innerHTML = influence[0];
      p2.innerHTML = influence[1];
    } else {
      p1.innerHTML = influence[1];
      p2.innerHTML = influence[0];
    }
  ...
  nullifyInfluenceToken: function (player, state) {
    const token = document.getElementById(`it-${state.id}`);
    token.remove();
    const board = document.getElementById("board");
    const newToken = document.createElement("div");
    newToken.classList.add("influenceToken");
    newToken.classList.add("p-" + player);
    newToken.style.left = `${state.location.x - 16}px`;
    newToken.style.top = `${state.location.y - 16}px`;
    board.appendChild(newToken);
  },

  drawDirectionArrows: function (border) {
    const arrows = document.querySelectorAll(".arrowHelper");
    arrows.forEach((arrow) => arrow.remove());
    const board = document.getElementById("board");
    const helper = document.createElement("div");
    helper.classList.add("arrowHelper");
    helper.style.left = `${border.location.x - 16}px`;
    helper.style.top = `${border.location.y - 16}px`;
    helper.style.transform = `rotate(${border.rotation}deg)`;
